...
export default function Cadastro() {
  ...
  async function submit(event) {
    ...
    try {
      const usuario = await usuarioApi.criar({ nome: form.nome, email: form.email, senha: form.senha, tipo_usuario: tipo });
      const id = usuario.data.id;
      if (tipo === 'CONSUMIDOR') await consumidorApi.criar({ fk_id: id, cpf: form.cpf });
      if (tipo === 'ESTABELECIMENTO') await estabelecimentoApi.criar({ fk_id: id, cnpj: form.cnpj, is_aberto: false });
      if (tipo === 'ENTREGADOR') await entregadorApi.criar({ fk_id: id, veiculo: form.veiculo, cnh: form.veiculo === 'MOTO' ? form.cnh : null, data_nascimento: form.data_nascimento, is_disponivel: true });
      navigate('/login');
    } catch {
